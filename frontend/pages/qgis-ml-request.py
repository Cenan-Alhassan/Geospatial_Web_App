...
with st.sidebar:
    ...
    st.info(
        "Ensure qgis-ml-server-flask.py is running. Note that only the owner of the server has access to this command."
    )
...
with st.form("scp_request_form"):
    ...
    bands_input = st.text_area(
        "Input Band Layers (One path per line)", value=default_bands_str, height=200
    )

    col1, col2 = st.columns(2)
    with col1:
        training_input = st.text_input(
            "Training Input (.scpx)", value=DEFAULT_PARAMS["TRAINING_INPUT_SCPX"]
        )
        testing_input = st.text_input(
            "Testing Input (Vector)", value=DEFAULT_PARAMS["TESTING_INPUT_SCPX"]
        )
        classifier_input = st.text_input(
            "Classifier Input (.rsmo) [Optional]",
            value=DEFAULT_PARAMS["CLASSIFIER_INPUT_RSMO"],
        )

    with col2:
        output_folder = st.text_input(
            "Classification Output Folder",
            value=DEFAULT_PARAMS["CLASSIFICATION_FOLDER"],
        )
        raster_output = st.text_input(
            "Raster Output Name", value=DEFAULT_PARAMS["RASTER_OUTPUT"]
        )
        nodata_val = st.number_input(
            "NoData Value (0 = None)",
            value=DEFAULT_PARAMS["NODATA"]
            if DEFAULT_PARAMS["NODATA"] is not None
            else 0.0,
        )
    ...
    with c3:
        mc_field = st.text_input(
            "Class/Macroclass Field", value=DEFAULT_PARAMS["MC_OR_CLASS_FIELD"]
        )
    ...
    calc_conf = b3.checkbox(
        "Calc Confidence", value=DEFAULT_PARAMS["CALCULATE_CONFIDENCE"]
    )
    cross_val = b4.checkbox(
        "Cross Validation", value=DEFAULT_PARAMS["CROSS_VALIDATION"]
    )
    sig_thresh_bool = b5.checkbox(
        "Signature Threshold", value=DEFAULT_PARAMS["SIGNATURE_THRESHOLD"]
    )
    ...
    with st.expander("Random Forest Settings"):
        ...
        rf_trees = rf_c1.number_input(
            "Number of Trees", value=DEFAULT_PARAMS["RF_TREES"]
        )
        rf_split = rf_c2.number_input(
            "Min Samples Split", value=DEFAULT_PARAMS["RF_SPLIT"]
        )
        rf_feats = st.text_input(
            "Max Features", value=DEFAULT_PARAMS["RF_MAX_FEATURES"]
        )
        rf_ovr = st.checkbox("One-Vs-Rest", value=DEFAULT_PARAMS["RF_ONE_VS_REST"])
        rf_balanced = st.checkbox(
            "Balanced Class Weight", value=DEFAULT_PARAMS["BALANCED_CLASS_WEIGHT"]
        )

    with st.expander("Support Vector Machine Settings"):
        svm_c1, svm_c2, svm_c3 = st.columns(3)
        svm_c = svm_c1.number_input(
            "Regularization (C)", value=float(DEFAULT_PARAMS["SVM_REGULARIZATION"])
        )
    ...
    with st.expander("Multi-Layer Perceptron Settings"):
        mlp_layers = st.text_input(
            "Hidden Layers (comma sep)", value=DEFAULT_PARAMS["MLP_LAYERS"]
        )
        mlp_c1, mlp_c2 = st.columns(2)
        mlp_iter = mlp_c1.number_input(
            "Max Iterations", value=DEFAULT_PARAMS["MLP_MAX_ITER"]
        )
        mlp_act = mlp_c2.text_input(
            "Activation", value=DEFAULT_PARAMS["MLP_ACTIVATION"]
        )
        mlp_alpha = st.number_input(
            "Alpha", value=DEFAULT_PARAMS["MLP_APLHA"], format="%.4f"
        )
        mlp_rate = st.number_input(
            "Learning Rate Init",
            value=DEFAULT_PARAMS["MLP_LEARNING_RATE_INIT"],
            format="%.4f",
        )
...
if submitted:
    ...
    payload = {
        ...
        "TRAINING_INPUT_SCPX": training_input
        if training_input
        else DEFAULT_PARAMS["TRAINING_INPUT_SCPX"],
        "TESTING_INPUT_SCPX": testing_input
        if testing_input
        else DEFAULT_PARAMS["TESTING_INPUT_SCPX"],
        "USE_MACROCLASS": use_macro,
        "MC_OR_CLASS_FIELD": mc_field
        if mc_field
        else DEFAULT_PARAMS["MC_OR_CLASS_FIELD"],
    ...
    try:
        ...
        st.error(
            "Could not connect to the server. Is `qgis-ml-server-flask.py` running?"
        )
    except requests.exceptions.Timeout:
        st.error(
            "The request timed out. The classification might still be running in the background."
        )
